...
def consultation():
    try:
        ...
        # 軽度の不安喚起メッセージを付与
        risk_warning = "なお、このまま放置すると、ページが正しく表示されなくなったり、信頼性の低下を招く可能性があります。"
        gpt_advice += f"\n\n{risk_warning}\n\n→ この問題について外注する場合は、見積もりをご希望されますか？"
        subsidy_info = "※本件は条件により、自治体や国の助成金・補助金を活用できる可能性もあります。申請サポートも承ります。"
        gpt_advice += f"\n\n{subsidy_info}"
